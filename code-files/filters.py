...
def applyfilter():
    ...
    if see_ph:
        ...
        if ph == "Peak hour":
            #data = data.loc[(data['Datetime'].dt.time >= datetime.time(7, 00, 00)) &
                            ...
                            # (data['Datetime'].dt.time >= datetime.time(17, 00, 00)) &
                            # (data['Datetime'].dt.time <= datetime.time(19, 00, 00))]
            st.write("Office Peak Hour filter applied")
        else:
            #data = data.loc[~((data['Datetime'].dt.time >= datetime.time(7, 00, 00)) &
                            #   (data['Datetime'].dt.time <= datetime.time(9, 00, 00)) |
                            #   (data['Datetime'].dt.time >= datetime.time(17, 00, 00)) &
                            #   (data['Datetime'].dt.time <= datetime.time(19, 00, 00)))]
            st.write("Off peak hour filter applied")
    ...
    if see_w:
        ...
        #data = data.loc[data['weather'] == wea]
        st.write("Weather filter applied")
    ...
    if see_l:
        ...
        #data = data.loc[data['LIGHT'] == li]
        st.write("Lighting Conditions filter applied")
    ...
    if see_rdsurf:
        ...
        #data = data.loc[data['RDSURF'] == rdsurf]
        st.write("Temporary road surface condition filter applied")
    ...
    if see_pav:
        ...
        #data = data.loc[data['SURF_TYP'] == pav]
        st.write("Pavement type filter applied")
    ...
    if see_acctyp:
        ...
        #data = data.loc[data['ACCTYPE'] == acctyp]
        st.write("Only Vehicle to Vehicle Collision filter applied")
